...
func main() {
	...
	//file_link := "https://images-1256261760.cos.ap-guangzhou.myqcloud.com/test3_01.mp3"
	//akID := os.Getenv("ALIYUN_ACCESSKEY_ID")
	//akSecret := os.Getenv("ALIYUN_ACCESSKEY_SECRET")
	//aKey := os.Getenv("ALIYUN_APPKEY")
	//
	//text, err := Voice(file_link, akID, akSecret, aKey)
	//fmt.Println(text, err)
}

//获取语音识别接口的返回结果，格式为json字符串
func Voice(file_link, akID, akSecret, aKey string) (json_str string, err error) {
	/**
	 * 地域ID
	 * 常量内容，请勿改变
	 */
	...
	*/
	...
	if (postResponse.GetHttpStatus() != 200) {
		...
		return json_str, errors.New("录音文件识别请求失败，Http错误码")
	}
	...
	if statusText == "SUCCESS" {
		...
		return json_str, errors.New("录音文件识别请求失败")
	}
	/**
	...
	*/
	...
	if statusText == "SUCCESS" || statusText == "SUCCESS_WITH_NO_VALID_FRAGMENT" {
		...
		return json_str, errors.New("录音文件识别失败")
	}
}
